...
class StandardView : public View{
 ...
 // StandardView(std::istream& in, std::ostream& out);
  PlayerConfiguration getPlayerConfiguration() override;
  ...
  virtual std::pair<int, int> getFiringCoordinate(const Player& attacker) override;
  virtual void showWinner(const Player& winner) override;
  virtual void showResultOfAttack(const Player& attacker,
                                    const AttackResult& attackResult) override;
